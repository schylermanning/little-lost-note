...
export default function PageRenderer({ page }: PageRendererProps) {
  ...
  switch (page.layout) {
    ...
    case 'multi-image':
      ...
      // Special case: Dedication page (id: 3) - centered attribution image taking up most of slide
      if (page.id === 3 && page.type === 'dedication' && page.imageSrc) {
        return (
          <div className="flex flex-col items-center justify-center min-h-[90vh]">
            <div className="relative w-full max-w-4xl min-h-[700px] md:min-h-[800px]">
              {renderImage(page.imageSrc, page.altText, 'min-h-[700px] md:min-h-[800px]')}
            </div>
          </div>
        );
      }
